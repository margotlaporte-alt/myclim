...
- A meeting record can still be achieved in a heat even when the heat does not decide the winner.
...
- `2023`: keep `60 m Women`, `60 m Hurdles Women`, and `60 m Hurdles Men` heats linked to their finals, and merge the `Continuation` pages back into the same final sections. Keep the two `DNS` rows in the `60 m Women` final continuation. Treat `400 m Men` as mixed timed heats with the global PDF ranking preserved while keeping each athlete's source heat (`I` / `II`). Treat `800 m Women`, `800 m Men`, `1500 m Women`, and `3000 m Men` as simple finals. Keep `Triple Jump Women`, `High Jump Women`, and `Shot Put Men` as field-event finals, but after `2006` store only the final result value and status, not the attempt-by-attempt detail as exploitable data. Keep the `4 / 4` tie in the `60 m Hurdles Women` final exactly as printed. Correct `VOORRIPS Suzanne` to `NED`, normalize `KIPLANGAT CHELANGAT Josephine`, and standardize notes such as `Meeting Record`, `World Lead`, and `National Record Indoor`.
- `2024`: ignore all standard `Pre-Programme` and `Post-Programme` youth/local events, but keep the `Special Olympics Mixte` races as dedicated disciplines (`200m - Special Olympics`, `400m - Special Olympics`). Keep `60 m Women`, `60 m Hurdles Women`, and `60 m Hurdles Men` heats linked to their finals, and merge all `Continuation` pages back into the same sections. Treat `200 m Men` as mixed timed heats with a recomputed overall classification from both heats while preserving the source heat (`1` / `2`). Treat `1500 m Women`, `1500 m Men`, `800 m Women`, and `3000 m Men` as single `Timed Final` events even when the PDF labels them `Timed Heat 1` or `Timed Heat`. Keep `Long Jump Women`, `Shot Put Men`, `Pole Vault Women`, and `High Jump Men` as field-event finals, but after `2006` store only the final result value and status, not attempt-by-attempt detail as structured exploitable data. Keep the double rank `3` in `60 m Hurdles Men` and `High Jump Men`, keep `MICHEAU Sébastien` as `NM`, and standardize notes such as `Meeting Record`, `National Record Indoor`, `Meeting Record` from `Record CMCM`, and `European Record U20`.
- `2025`: ignore all `U12` / `U14` youth events, both `60 m NATIONAL` blocks, and the `Relais Australien` pages. Keep `200m - Special Olympics` as a separate dedicated discipline with `gender = X`. Keep `60 m Women`, `60 m Hurdles Women`, and `60 m Hurdles Men` heats linked to their finals, and preserve the `60 m Women` heat times at the thousandth when the PDF provides them (`7.394`, `7.398`, `7.399`). Keep `60 m Hurdles Women` `A Final` and `B Final` as separate stored sections, with the official winner coming from `A Final`. Treat `400 m Men` and `200 m Women` as mixed timed finals with the global PDF ranking preserved while keeping each athlete's source heat (`I` / `II`). Treat `800 m Men`, `1500 m Women`, `1500 m Men`, and `3000 m Men` as single timed finals. Keep `Long Jump Women`, `Shot Put Men`, and `Pole Vault Women` as field-event finals, but after `2006` store only the final result value and status, not attempt-by-attempt detail as structured exploitable data. Keep the tied rank `5` in both the `60 m Women` final and the `60 m Hurdles Men` final, keep `TR17.1.2` as a note on the `DQ` in `400 m Men`, and distinguish normalized notes such as `Meeting Record`, `National Record`, `National Record Indoor`, and `National Record Indoor U23`.
- `2026`: ignore all `U12` / `U14` youth events, the `60m Influencer Race`, and the `4x200m Business Relay`. Keep `200m - Special Olympics` as a separate dedicated discipline with `gender = X`. Keep `60 m Women`, `60 m Hurdles Women`, and `60 m Hurdles Men` heats linked to their finals. Preserve the exact thousandths when the PDF gives them either in parentheses or on the next line (`7.280`, `8.120`, `8.122`, `7.270`, `7.350`, etc.). Treat `400 m Men` and `200 m Women` as mixed timed finals with the global PDF ranking preserved while keeping each athlete's source heat (`I` / `II`). Treat `800 m Women`, `800 m Men`, `3000 m Men`, `1500 m Women`, and `1500 m Men` as single finals. Keep `Long Jump Men` and `Pole Vault Women` as field-event finals, but after `2006` store only the final result value and status, not attempt-by-attempt detail as structured exploitable data. Normalize inline and standalone notes such as `MR` / `Record de meeting` -> `Meeting Record`, `NR` on indoor track events -> `National Record Indoor`, `MR,NR` / `=MR,NR` -> combined meeting + indoor national record, and `NR Lux. U20` -> `National Record Indoor U20`. Normalize country codes to official forms such as `MAR` for Morocco and `LBR` for Liberia.
...
- `meetingWinners` must always come from the official winning section of the event, not from a heat.
- `meetingRecords` can come from a heat, semifinal, or final if the mark is official in the PDF/result set.
- If two athletes truly share the same best official meeting mark, keep a co-record instead of forcing a single holder.
- Reference correction confirmed by user: `2021` `50 m Women` meeting record at `6.27` belongs to `MONTAG Jennifer`, not `GNAFOUA Floriane`.
