...
export function SiteLayout() {
  ...
  const latestEdition = editions.find((e) => !e.isClosed) ?? editions[0] ?? null;
  const isLive = !!(latestEdition?.streamingUrl || latestEdition?.liveResultsUrl);
  ...
  return (
    <div className="site-page">
      ...
      <nav className={`site-nav${scrolled || mobileOpen ? " site-nav--scrolled" : ""}`}>
        <div className="site-nav__inner">
          ...
          <div className="site-nav__actions">
            {isLive && (
              <a href="/#live-results" className="site-nav__live-badge">
                <span className="site-nav__live-dot" />
                LIVE
              </a>
            )}
            {currentUser ? (
      ...
      <div ref={mobileRef} className={`site-nav__mobile${mobileOpen ? " site-nav__mobile--open" : ""}`}>
        {isLive && (
          <a href="/#live-results" className="site-nav__mobile-link site-nav__mobile-live">
            <span className="site-nav__live-dot" /> LIVE
          </a>
        )}
        {NAV_LINKS.map((link) => (
          <NavLink
