...
// ─── Reimport results for specific years from bundled JSON ───────────────────

/**
 * Deletes all meetingResults for the given years, then re-seeds from JSON.
 * Idempotent — safe to re-run. Calls onProgress with status messages.
 */
export async function reimportResultsForYears(years, onProgress) {
  const resultsJson = await import("../data/meetingResults.json").then((m) => m.default);
  let totalDeleted = 0;
  let totalInserted = 0;

  for (const year of years) {
    onProgress?.(`${year}: suppression des anciens résultats…`);
    const deleted = await clearResultsForYear(year);
    totalDeleted += deleted;

    const yearData = resultsJson[String(year)];
    if (!yearData?.length) {
      onProgress?.(`${year}: aucune donnée dans le JSON, ignoré.`);
      continue;
    }

    onProgress?.(`${year}: import de ${yearData.length} résultats…`);
    let batch = writeBatch(db);
    let count = 0;
    ...
    await batch.commit();
    totalInserted += count;
    onProgress?.(`${year}: ✅ ${count} résultats importés.`);
  }

  return `Terminé — ${totalDeleted} supprimés, ${totalInserted} insérés.`;
}
...
export async function updateEdition(year, fields) {
  await updateDoc(doc(db, MEETING_EDITIONS_COL, String(year)), fields);
}
