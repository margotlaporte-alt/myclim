...
function MeetingHistoryPage({ Panel }) {
  ...
  const [streamingUrlSaving, setStreamingUrlSaving] = useState(false);
  const [liveResultsUrlInput, setLiveResultsUrlInput] = useState("");
  const [liveResultsUrlSaving, setLiveResultsUrlSaving] = useState(false);
    ...
    <div className="page">
      ...
      <section className="panel-grid panel-grid--1">
        <Panel title="Select edition">
          ...
          {isAdmin && selectedEdition && !selectedEdition.cancelled && (
            <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#374151" }}>
                🎥 URL streaming (YouTube embed)
              </label>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <input
                  type="text"
                  placeholder="https://www.youtube.com/embed/..."
                  value={streamingUrlInput !== "" ? streamingUrlInput : (selectedEdition.streamingUrl || "")}
                  onChange={(e) => setStreamingUrlInput(e.target.value)}
                  style={{
                    flex: 1, padding: "0.35rem 0.6rem", fontSize: "0.82rem",
                    border: "1px solid #d1d5db", borderRadius: 6, fontFamily: "monospace",
                  }}
                />
                <button
                  className="btn btn--secondary"
                  disabled={streamingUrlSaving}
                  style={{ whiteSpace: "nowrap", fontSize: "0.82rem" }}
                  onClick={async () => {
                    setStreamingUrlSaving(true);
                    await updateEdition(selectedEdition.year, { streamingUrl: streamingUrlInput });
                    setStreamingUrlInput("");
                    setStreamingUrlSaving(false);
                  }}
                >
                  {streamingUrlSaving ? "Sauvegarde…" : "Sauvegarder"}
                </button>
                {selectedEdition.streamingUrl && (
                  <button
                    className="btn btn--secondary"
                    disabled={streamingUrlSaving}
                    style={{ whiteSpace: "nowrap", fontSize: "0.82rem", color: "#dc2626" }}
                    onClick={async () => {
                      setStreamingUrlSaving(true);
                      await updateEdition(selectedEdition.year, { streamingUrl: "" });
                      setStreamingUrlInput("");
                      setStreamingUrlSaving(false);
                    }}
                  >
              ...
              {selectedEdition.streamingUrl && (
                <span style={{ fontSize: "0.75rem", color: "#6b7280" }}>
                  Actuel : <code style={{ background: "#f3f4f6", padding: "1px 4px", borderRadius: 3 }}>{selectedEdition.streamingUrl}</code>
                </span>
              )}
            </div>
          )}
          ...
          {isAdmin && selectedEdition && !selectedEdition.cancelled && (
            <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#374151" }}>
                📊 URL live results (laportal)
              </label>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <input
                  type="text"
                  placeholder="https://fla.laportal.net/Competitions/Details/..."
                  value={liveResultsUrlInput !== "" ? liveResultsUrlInput : (selectedEdition.liveResultsUrl || "")}
                  onChange={(e) => setLiveResultsUrlInput(e.target.value)}
                  style={{
                    flex: 1, padding: "0.35rem 0.6rem", fontSize: "0.82rem",
                    border: "1px solid #d1d5db", borderRadius: 6, fontFamily: "monospace",
                  }}
                ...
                <button
                  className="btn btn--secondary"
                  disabled={liveResultsUrlSaving}
                  style={{ whiteSpace: "nowrap", fontSize: "0.82rem" }}
                  onClick={async () => {
                    setLiveResultsUrlSaving(true);
                    await updateEdition(selectedEdition.year, { liveResultsUrl: liveResultsUrlInput });
                    setLiveResultsUrlInput("");
                    setLiveResultsUrlSaving(false);
                  }}
                >
                  {liveResultsUrlSaving ? "Sauvegarde…" : "Sauvegarder"}
                </button>
                {selectedEdition.liveResultsUrl && (
                  <button
                    className="btn btn--secondary"
                    disabled={liveResultsUrlSaving}
                    style={{ whiteSpace: "nowrap", fontSize: "0.82rem", color: "#dc2626" }}
                    onClick={async () => {
                      setLiveResultsUrlSaving(true);
                      await updateEdition(selectedEdition.year, { liveResultsUrl: "" });
                      setLiveResultsUrlInput("");
                      setLiveResultsUrlSaving(false);
                    }}
                  >
              ...
              {selectedEdition.liveResultsUrl && (
                <span style={{ fontSize: "0.75rem", color: "#6b7280" }}>
                  Actuel : <code style={{ background: "#f3f4f6", padding: "1px 4px", borderRadius: 3 }}>{selectedEdition.liveResultsUrl}</code>
                </span>
              )}
            </div>
          )}
        ...
        <section className="panel-grid panel-grid--1">
          <Panel title="Admin — Database" subtitle="Import and manage historical data">
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: "0.75rem" }}>
                <p className="panel-note" style={{ marginBottom: "0.5rem" }}>
                  <strong>Réimporter les résultats 2009–2018</strong> — supprime les résultats Firestore pour ces années
                  et les réimporte depuis le JSON bundlé. À utiliser si des erreurs d'import ont été détectées pour ces éditions.
                </p>
                <button
                  className="btn btn--danger"
                  style={{ fontSize: "0.82rem" }}
                  ...
                  onClick={async () => {
                    ...
                    if (!window.confirm(`Supprimer et réimporter les résultats pour ${years.join(", ")} ?`)) return;
                    setSeedLog([]);
                    setSeeding(true);
                    try {
                      const result = await reimportResultsForYears(years, (msg) => setSeedLog((p) => [...p, msg]));
                      setSeedLog((p) => [...p, `✅ ${result}`]);
                    } catch (err) {
                      setSeedLog((p) => [...p, `❌ ${err.message}`]);
                    } finally {
                ...
                >
                  🔄 Réimporter résultats 2009–2018
                </button>
              </div>
              <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: "0.75rem" }}>
                <p className="panel-note" style={{ marginBottom: "0.5rem" }}>
